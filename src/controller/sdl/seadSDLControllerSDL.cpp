...
const u8 SDLController::cDefaultVKeyConfig[Controller::PadIdx::eMax] = {
    'A',        // PadIdx::eA
    'B',        // PadIdx::eB
    'C',        // PadIdx::eC
    'X',        // PadIdx::eX
...
const u8 SDLController::cDefaultJoyPadConfig[Controller::PadIdx::eMax] = {
    4,  // PadIdx::eA
    3,  // PadIdx::eB
    7,  // PadIdx::eC
    2,  // PadIdx::eX
...
SDLController::SDLController(ControllerMgr* mgr, s32 joyPadNumber)
    : Controller(mgr)
...
void SDLController::setVKeyConfig(const u8* keyConfig)
{
    MemUtil::copy(mVKeyConfig, keyConfig, PadIdx::eMax);
}

void SDLController::setVKeyConfig(PadIdx bit, u8 vkey)
{
    if (bit >= PadIdx::eMax)
    {
...
void SDLController::setVKeyConfigDefault()
{
    MemUtil::copy(mVKeyConfig, cDefaultVKeyConfig, PadIdx::eMax);
}

bool SDLController::isJoyPadEnable() const
{
    // TODO
    return false;
}

void SDLController::getStickClamp(s32* min, s32* max) const
{
    SEAD_ASSERT(min);
    SEAD_ASSERT(max);
...
void SDLController::setStickClamp(s32 min, s32 max)
{
...
void SDLController::setJoyPadConfig(const u8* joyPadConfig)
{
    MemUtil::copy(mJoyPadConfig, joyPadConfig, PadIdx::eMax);
}

void SDLController::setJoyPadConfig(PadIdx bit, u8 no)
{
    if (bit >= PadIdx::eMax)
    {
        SEAD_ASSERT_MSG(false, "range over[%d]\n", static_cast<s32>(bit));
...
void SDLController::setJoyPadConfigDefault()
{
    MemUtil::copy(mJoyPadConfig, cDefaultJoyPadConfig, PadIdx::eMax);
}

void SDLController::calcImpl_()
{
    ...
    SDLJoyPadDevice* jpDevice = DynamicCast<SDLJoyPadDevice>(mMgr->getControlDevice(ControllerDefine::DeviceId::eSDLJoyPad));
