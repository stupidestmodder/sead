...
protected:
    ...
    std::FILE** getHANDLE_(HandleBase* handle) const
    {
        HandleBuffer& handleBuffer = getHandleBaseHandleBuffer_(handle);
        return reinterpret_cast<std::FILE**>(&handleBuffer[0]);
    }

    void setHANDLE_(HandleBase* handle, std::FILE* handleInner) const
    {
        std::FILE** ptr = getHANDLE_(handle);
        *ptr = handleInner;
    }
