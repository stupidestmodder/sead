...
class GameFrameworkBaseSDL : public GameFramework
{
    SEAD_RTTI_OVERRIDE(GameFrameworkBaseSDL, GameFramework);
...
public:
    ...
    struct CreateArg
    {
        CreateArg()
            ...
            , wait_vblank(1)
            , fps(60.0f)
            , window_name("GameFrameworkSDL")
            , window_ex_style(SDL_WINDOW_HIDDEN)
        ...
        SDL_WindowFlags window_ex_style;
        Color4f clear_color;
        bool create_default_framebuffer;
    };

public:
    explicit GameFrameworkBaseSDL(const CreateArg& arg);
    ~GameFrameworkBaseSDL() override;
...
public:
    static GameFrameworkBaseSDL* sInstance;

    sead::Heap* getSDLHeap()
    {
        return mSDLHeap;
    }
